...
async function run(url) {
  ...
  for (const scheme of ['light', 'dark']) {
    ...
    // HERE has to take a tap while the sheet is in peek, which is
    ...
    // centre returned the legal strip and a tap on HERE did nothing. Measured
    ...
    ok(`phone/${scheme}: HERE takes a tap over the peek sheet`,
    ...
    // With the sheet shut HERE sits at the bottom right and the time bar
    // runs along the bottom; they used to overlap by the bar's right end.
    // Measured as rectangles, in the state where both are on screen.
    const bar = await p.evaluate(async () => {
      ...
      const tb = R('timebar'), here = R('here');
      const visible = getComputedStyle(document.getElementById('timebar')).display !== 'none';
      return { visible, hereOverBar: hit(here, tb), tb, here };
    });
    ok(`phone/${scheme}: HERE does not sit on the time bar`,
       bar.visible && !bar.hereOverBar, JSON.stringify(bar));
