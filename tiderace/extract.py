...
def awaiting(path: str = REVIEW_PATH, limit: int = 200,
    ...
    """What is in force, one row per observation and witness, with its id and
    what it is still doing to the forecast.
    ...
    """
    queued = [r for r in load_queue(path)
              if r.get("kind") != "regulation"
              and r.get("status") in ("applied", "on_file", "pending")]
    credited = {_observation(r) for r in queued if _norm_name(r.get("attributed_to"))}
